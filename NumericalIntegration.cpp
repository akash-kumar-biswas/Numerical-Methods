...
double simpson_1_3_rule(double a,double b,int n) {
    if (n%2!=0){
        cout<<"Simpson's 1/3 rule requires an even number of subintervals!"<<endl;
        return -1; 
    }
...
double simpson_3_8_rule(double a, double b, int n) {
    if (n%3!=0){
        cout<<"Simpson's 3/8 rule requires the number of subintervals to be a multiple of 3"<<endl;
        return -1;
    }
